...
type Request struct {
	...
	TypeOfAccess           int	    `json:"typeOfAccess"`
	Description            string       `json:"description"`
...
func (c *HealthContract) AnswerRequest(ctx contractapi.TransactionContextInterface,
	response int, requestID, socialSecurityNumber string) error {
	...
	if err != nil {
   	...
   	// Find and update the targetRequest within the patientWallet
    	found := false
    	for i, req := range patientWallet.Requests {
        	if req.RequestID == requestID && req.Status != response {
            	patientWallet.Requests[i].Status = response
            	patientWallet.Requests[i].StatusChangedDate = time.Now().Unix()
            	found = true
            	//break
                }
    	}
    	...
    	if !found {
       		 return fmt.Errorf("request ID %s not found in patient's requests", requestID)
    	}
...
func checkIfHealthcareProfessionalHaveAccess(patientWallet PatientWallet,
	...
	for _, request := range patientWallet.Requests {
		if request.Organization == organization &&
			...
			request.ExpirationDate <= time.Now().Unix() { // Inverter o sinal quando tivermos em PRD.
			if request.Status == 1 {
				return nil
			}
		}
